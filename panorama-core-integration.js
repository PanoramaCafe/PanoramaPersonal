/* Panorama Personal — synchronization engine.
   SAFETY MODE: local attendance is never deleted by a remote snapshot.
   States are merged by record id before synchronization. */
(function(){
'use strict';
...
function mergeById(localList,remoteList){
  const out=new Map();
  for(const item of (remoteList||[])){if(item?.id)out.set(String(item.id),clone(item))}
  for(const item of (localList||[])){if(item?.id)out.set(String(item.id),clone(item))}
...
function mergeStates(local,remote){
  ...
  const arrayKeys=['employees','sessions','payments','payrollPeriods','weekFinalizations','approvals'];
  for(const key of arrayKeys)out[key]=mergeById(local[key],remote[key]);
...
async function sync(){if(syncing||applying||!navigator.onLine)return false;syncing=true;try{const local=read(CORE_STORE);const queued=read(QUEUE);const row=await getRemote();const remote=row?.data||null;const base=queued||local;const merged=mergeStates(base,remote);if(!merged)return true;const needsWrite=!remote||!same(merged,remote);const outgoing=needsWrite?stamp(merged):merged;if(needsWrite){const saved=await put(outgoing);apply(saved?.data||outgoing);try{await publishPayments(saved?.data||outgoing)}catch(e){console.warn(e)}}else if(!same(merged,local))apply(merged);localStorage.removeItem(QUEUE);refresh();return true}catch(e){console.warn('Sincronización pendiente',e);refresh();return false}finally{syncing=false;refresh()}}
...
window.PanoramaPersonalSync={sync:()=>sync(),status:()=>({online:navigator.onLine,pending:!!read(QUEUE)})};
