/* Panorama Personal — synchronization engine.
   Supabase is authoritative. Local changes are queued while offline.
   Conflict protection compares snapshot timestamps so a stale device cannot
   overwrite newer server data. No employee or attendance data is deleted here. */
(function(){
'use strict';
...
function stampValue(data){return Date.parse(data?._sync?.updatedAt||'')||0}
...
async function sync(force=false){if(syncing||applying||!navigator.onLine)return false;syncing=true;try{const queued=read(QUEUE);const row=await getRemote();if(queued){const remote=row?.data;const queuedTime=stampValue(queued),remoteTime=stampValue(remote);if(remote&&remoteTime>queuedTime){apply(remote);localStorage.removeItem(QUEUE);refresh();return true}const outgoing=queuedTime?queued:stamp(queued);const saved=await put(outgoing);const authoritative=saved?.data||outgoing;apply(authoritative);try{await publishPayments(authoritative)}catch(e){console.warn(e)}localStorage.removeItem(QUEUE);refresh();return true}if(row?.data){const remote=clone(row.data);if(force||!same(remote,read(STORE)))apply(remote);localStorage.removeItem(QUEUE);refresh();return true}const local=read(STORE);if(local){const outgoing=stamp(local);const saved=await put(outgoing);apply(saved?.data||outgoing);localStorage.removeItem(QUEUE)}return true}catch(e){console.warn('Sincronización pendiente',e);refresh();return false}finally{syncing=false;refresh()}}
function observe(){last=JSON.stringify(read(STORE));setInterval(()=>{if(applying)return;const now=read(STORE),raw=JSON.stringify(now);if(raw!==last){last=raw;if(now){const queued=stamp(now);localStorage.setItem(QUEUE,JSON.stringify(queued));refresh();if(navigator.onLine)setTimeout(()=>sync(true),150)}}},350)}
