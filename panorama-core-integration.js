/* Panorama Personal <-> Panorama Café Core
   Ruta estable:
   - localStorage conserva la respuesta inmediata de la app.
   - Los cambios locales se publican en Supabase.
   - Los pagos se publican hacia Finanzas.
   - Cada 5 segundos se comprueba el estado remoto.
   - Si cambió en otro dispositivo, se actualiza y recarga automáticamente.
*/
(function(){
  ...
  async function publishPayments(data){
    ...
    for(const p of payments){
      ...
      const upsert=await fetch(cfg.url+'/rest/v1/panorama_payroll_payments?on_conflict=id',{
  ...
  async function pull(initial=false){
    ...
    try{
      ...
      if(JSON.stringify(current)!==JSON.stringify(row.data)){
        console.info('Panorama Personal: cambio remoto detectado, actualizando interfaz');
        writeLocal(row.data);
        location.reload();
      }
