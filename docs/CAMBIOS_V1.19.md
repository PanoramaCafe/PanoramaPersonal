...
- Nuevo `panorama-session.js`: inicio de sesión de la nube (Supabase Auth) para la tablet, con renovación automática del token. Botón en Administración → Ajustes → 🔐 Sesión de la nube.
- Sync y puente de pagos envían el token cuando hay sesión (si no, siguen con la clave pública, compatible con hoy).
- Si la nube rechaza la conexión (401/403), la esquina muestra "Iniciar sesión de la nube" (tocable).
- Service worker v10.
- Guía para cerrar el acceso público: `docs/SUPABASE_PENDIENTE.md` (sección 3).
